...
static inline void pio_cfg_input(uintptr_t base, unsigned int pin, int use_pullup)
{
...
static inline void pio_cfg_A_periph(uintptr_t base, unsigned int pin, int use_pullup)
{
  outw(base+PIO_IDR, pin);
  outw(base+ (use_pullup ? PIO_PPUER : PIO_PPUDR), pin);
...
static struct sam9_nand_config ek_nand_config = {
  ...
  .smc_mode  =  (3<<16)|AT91C_SMC_READMODE|AT91C_SMC_WRITEMODE|AT91C_SMC_NWAITM_NWAIT_DISABLE,
					  ...
					  const unsigned char *spare, unsigned spareLength)
{
  ...
  memcpy(spare_buf+chip->ecclayout->oobfree[0].offset, spare, spareLength);
 ...
 */
static int atmel_nand_calculate(const unsigned char *dat, size_t sz, unsigned char *ecc_code)
{
	...
	ecc_value = ecc_readl(ek_nand_config.io_addr_ecc_base, NPR) & ATMEL_ECC_NPARITY;
		...
		unsigned char *read_ecc, unsigned char *isnull)
{
	...
	ecc_bit = ecc_readl(ek_nand_config.io_addr_ecc_base, PR) & ATMEL_ECC_BITADDR;
	/* get word address (12 bits) */
	ecc_word = ecc_readl(ek_nand_config.io_addr_ecc_base, PR) & ATMEL_ECC_WORDADDR;
	ecc_word >>= 4;
  ...
  while(nii->uNandID){
    if(nii->uNandID == chipID){
      if(nii->uNandSpareSize!=0x40){
        kprintf("only support 64byte spare area nand\n");
        break;
      }
      ek_nand_config.info = nii;
      READ_NAND(); //don't care
      unsigned char info = READ_NAND();
      kprintf("Nandflash Detected: chipID: 0x%04x, type: %s, byte3: 0x%02x\n", chipID, nii->name, info);
      goto found;
    }
    nii++;
  }
...
found:
  ...
  switch (chip.pg_size) {
		case 2048:
			...
			ecc_writel(ek_nand_config.io_addr_ecc_phy_base, MR, ATMEL_ECC_PAGESIZE_2112);
			break;
    default:
  ...
  kprintf("Nandflash size: %dMB, spare area: %dByte, pg_size: %dByte.\n", 
      chip.pg_size*chip.pages_per_blk*chip.blk_cnt/1024/1024, chip.spare_size, chip.pg_size); 
  return 1;
}
...
void nandflash_hw_init(void)
{
   ...
   outw(AT91C_BASE_MATRIX+AT91_MATRIX_EBICSA,  (csa|AT91_MATRIX_EBI_CS3A));
...
//    kprintf("# %08x\n", *(volatile unsigned*)(AT91C_BASE_PIOC+PIO_PDSR));
    ...
    unsigned short chipId = (((unsigned short)bManufacturerID)<<8)|bDeviceID;
...
{
    outw(AT91C_BASE_SMC + SMC_CTRL3,inw(AT91C_BASE_SMC + SMC_CTRL3) | AT91C_SMC_DBW_WIDTH_SIXTEEN_BITS);
}
...
{
    outw( AT91C_BASE_SMC + SMC_CTRL3, (inw(AT91C_BASE_SMC + SMC_CTRL3) & ~(AT91C_SMC_DBW)) |
           AT91C_SMC_DBW_WIDTH_EIGTH_BITS);
}
