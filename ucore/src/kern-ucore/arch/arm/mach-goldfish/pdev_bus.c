...
void pdev_bus_init()
{
  ...
  while(1){
    ...
    if(device.name_len < 61){
      outw(GOLDFISH_PDEV_BUS+PDEV_BUS_GET_NAME, device.name);
      device.name[device.name_len] = 0;
    }
    kprintf("  Dev: %d, %s, base=0x%08x, size=0x%x, irq=%d, irq_count=%d\n",
    device.id, device.name[0]?device.name:"<TOO LONG>", device.base, 
    device.size, device.irq, device.irq_count);
