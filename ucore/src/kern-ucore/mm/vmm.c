...
int
mm_unmap_keep_pages(struct mm_struct *mm, uintptr_t addr, size_t len) {
	uintptr_t start = ROUNDDOWN(addr, PGSIZE), end = ROUNDUP(addr + len, PGSIZE);
	if(!USER_ACCESS(start, end)) {
		return -E_INVAL;
	}
  ...
  if (ptep_invalid(ptep)) {
	if(vma->mfile.file != NULL) {
		...
		if(maddr == NULL) {
			...
			page_insert_pte(mm->pgdir, page, ptep, addr, perm & ~PTE_W);
...
#ifndef CONFIG_NO_SWAP
		assert(page_ref(page) + swap_page_count(page) == 1);
#else
