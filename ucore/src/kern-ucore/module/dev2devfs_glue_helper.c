...
void *__ucore_create_linux_dentry(dev_t devno, mode_t mode, const struct file_operations *fops)
{
...
int __ucore_linux_inode_fops_stub_open(struct ucore_device *dev, uint32_t open_flags)
{
  assert(dev->linux_dentry);
...
int __ucore_linux_inode_fops_stub_ioctl(struct ucore_device* dev, unsigned int cmd, unsigned long arg)
{
  int ret = -EINVAL;
